...
public class AsrMain {
    ...
    String SECRET_KEY;
    @Value("${baidu.apikeyrecord}")
    String APP_KEY_RECORD;
    @Value("${baidu.secretkeyrecord}")
    String SECRET_KEY_RECORD;
...
//    }
    ...
    public String runJsonPostMethod(String FILENAME) throws DemoException, IOException {
        TokenHolder holder = new TokenHolder(APP_KEY_RECORD, SECRET_KEY_RECORD, SCOPE);
        holder.resfresh();
        String token = holder.getToken();
        byte[] content = getFileContent(FILENAME);
